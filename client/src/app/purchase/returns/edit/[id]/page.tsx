...
export default function EditPurchaseReturnPage() {
  ...
  const { data: returnData, isLoading: isReturnLoading } = useQuery({
    ...
    queryFn: () => PurchaseReturnService.getPurchaseReturn(id),
  });
  ...
  React.useEffect(() => {
    if (returnData) {
      ...
      setPaymentMethod(returnData.payment_method || 'Cash');
      setStatus(returnData.status || 'completed');
      ...
      if (returnData.invoice_id) {
        setSelectedInvoice({ id: returnData.invoice_id, invoice_no: returnData.invoice_no });
      }
    }
  }, [returnData]);

  const updateMutation = useMutation({
    mutationFn: (data: any) => PurchaseReturnService.putPurchaseReturn(id, data),
    onSuccess: () => {
  ...
  const grandTotalRaw = subtotal + totalTax + totalCharges;
          ...
          <Card sx={{ borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9', mb: 3 }}>
            <CardContent>
              ...
              <Stack spacing={2}>
                ...
                >
                  {updateMutation.isPending ? 'Updating...' : 'Update Purchase Return'}
                </Button>
              </Stack>
            </CardContent>
          </Card>
