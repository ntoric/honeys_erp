...
export default function PaymentOutPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['payments-out', searchTerm],
    queryFn: () => PaymentsService.getPayments('pay', undefined, undefined, undefined, undefined, 1, 100)
  });
  ...
  const columns: GridColDef[] = [
    { field: 'payment_date', headerName: 'Date', width: 130, renderCell: (params) => {
      const date = params.value || params.row.PaymentDate || params.row.paymentDate;
      return date ? new Date(date).toLocaleDateString() : '-';
    }},
    { field: 'payment_no', headerName: 'Payment ID', width: 150, renderCell: (params) => (
      <Typography sx={{ fontWeight: 700, color: theme.palette.primary.main }}>
        {params.value || params.row.PaymentNo || params.row.paymentNo || 'N/A'}
      </Typography>
    )},
    ...
    { field: 'AmountSettled', headerName: 'Amount Settled', width: 150, renderCell: (params) => {
        ...
        return <Typography sx={{ fontWeight: 600, color: '#64748b' }}>₹{(amount + discount).toLocaleString()}</Typography>;
    }},
    { field: 'amount', headerName: 'Amount Paid', width: 150, renderCell: (params) => {
      const amount = params.value ?? params.row.Amount ?? 0;
      return (
        <Typography sx={{ fontWeight: 800 }}>₹{amount.toLocaleString()}</Typography>
      );
    }},
    { field: 'mode', headerName: 'Payment Mode', width: 150, renderCell: (params) => (
      <Chip 
  ...
  const totalPaid = payments.reduce((acc: number, curr: any) => acc + (curr.status !== 'cancelled' ? curr.amount : 0), 0);
      ...
      <Box sx={{ 
        ...
        <Box>
          ...
          <Typography sx={{ color: '#64748b', fontWeight: 500 }}>
            Record and manage payments made to your vendors.
          </Typography>
        </Box>
