...
import { SalesService, SalesInvoice } from '@/api';
...
export default function SalesInvoicesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['sales-invoices', searchTerm, statusFilter, fromDate, toDate],
  ...
  const handleExport = (exportData: any[]) => {
    ...
    const csvRows = [
      ...
      ...exportData.map(inv => [
        ...
        `"${inv.party_name || inv.partyName || inv.customer_name}"`,
        inv.party_mobile || inv.partyMobile || inv.customer_phone,
  ...
  const columns: GridColDef[] = [
    { 
      ...
      align: 'left',
      renderCell: (params) => (
        <Typography sx={{ fontWeight: 700, color: theme.palette.primary.main, fontSize: '0.875rem' }}>
          {params.value || params.row.invoiceNo || 'N/A'}
        </Typography>
      )
    },
    { 
      ...
      renderCell: (params) => (
        <Typography sx={{ fontSize: '0.875rem', fontWeight: 500 }}>
          {params.value || params.row.invoiceDate || 'N/A'}
        </Typography>
      )
    },
    { 
      ...
      align: 'left',
      renderCell: (params) => {
        const name = params.value || params.row.partyName || params.row.customer_name || 'Walk-in Customer';
        const mobile = params.row.party_mobile || params.row.partyMobile || params.row.customer_phone || '';
        return (
    ...
    { 
      ...
      renderCell: (params) => (
        <Typography sx={{ fontSize: '0.875rem', color: '#64748b' }}>
          {params.value || params.row.dueDate || 'N/A'}
        </Typography>
      )
    },
    { 
    ...
    { 
      ...
      renderCell: (params) => {
        ...
        else if (status === 'unpaid') { color = '#f59e0b'; bgColor = '#fffbeb'; }
  ...
  const summary = (data as any)?.summary || { total_sales: 0, paid_amount: 0, unpaid_amount: 0, cancelled_count: 0 };
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 5 }}>
        <Box>
          ...
          <Typography sx={{ color: '#64748b', fontWeight: 500 }}>
            Create and manage invoices for your sales transactions.
          </Typography>
        </Box>
      ...
      >
        <MenuItem onClick={() => { router.push(`/sales/invoices/view/${activeRowId}`); handleMenuClose(); }}>
